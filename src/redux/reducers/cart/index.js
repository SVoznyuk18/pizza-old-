...
const addPizzaToCartSuccess = (state, action) => {
  ...
  let { totalPrice, totalAmount } = state;
...
const increasePizzaAmountSuccess = (state, action) => {
  ...
  totalPrice = cart.reduce((accum, item) => (accum + item.price * item.amountPizzas), 0);
  totalAmount = cart.reduce((accum, item) => (accum + item.amountPizzas), 0);
...
const deletePizzaItemSuccess = (state, action) => {
  ...
  totalPrice = newCart.reduce((accum, item) => (accum + item.price * item.amountPizzas), 0);
  totalAmount = newCart.reduce((accum, item) => (accum + item.amountPizzas), 0);
